...
function renderSharedUsers(
  ...
  users
    ...
    .forEach((entry) => {
      ...
      wrapper.className =
        "flex flex-col gap-3 p-4 mb-4 rounded-xl border border-gray-200 dark:border-surface-600 bg-white dark:bg-surface-800 shadow-sm";

      wrapper.innerHTML = `
        <div class="flex items-center justify-between mb-2">
          <div class="font-medium text-surface-800 dark:text-white text-sm">${
            sharedUser.email || sharedUser.username
          }</div>
          ${
            isSelf
              ? ""
              : `<button data-user="${sharedUser._id}" class="remove-user-btn text-red-600 hover:text-red-800"><i class="fas fa-trash"></i></button>`
          }
        </div>
          ...
          ${Object.entries(permissions)
            .map(([key, val]) => {
              ...
              <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" id="${id}" class="peer hidden perm-checkbox" data-user="${
                sharedUser._id
              }" data-perm="${key}" ${val ? "checked" : ""} ${
                isSelf ? "disabled" : ""
              }>
  ...
  list.querySelectorAll(".perm-checkbox").forEach((checkbox) => {
    checkbox.addEventListener("change", async () => {
      ...
      const identifier =
        users.find((sw) => sw.user._id === userId)?.user.email || userId;
      const updated = {};
  ...
  list.querySelectorAll(".remove-user-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const identifier = users.find((sw) => sw.user._id === btn.dataset.user)
        ?.user.email;
...
function renderCalendarList() {
  ...
  if (currentFilter === "mine") {
    ...
    filtered = filtered.filter((c) =>
      c.sharedWith?.some((sw) => sw.user === user._id)
    );
  }
  ...
  if (filtered.length === 0) {
    calendarList.innerHTML =
      '<p class="text-surface-500">Nav atrastu kalendāru</p>';
  } else {
...
function renderCalendar(calendar) {
  ...
  el.className =
    "bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl px-5 py-4 transition hover:shadow-sm flex flex-col gap-2";
  el.innerHTML = `
  ...
  el.querySelector(".edit-btn")?.addEventListener("click", () => {
    ...
    document.getElementById(
      "editModalCalendarName"
    ).textContent = `"${calendar.name}"`;
    updateColorButtons(
      ".edit-color-btn",
      document.getElementById("editCalendarColor")
    );
...
function setupFilterHandlers() {
  document
    ...
    ?.addEventListener("input", renderCalendarList);
  document
    .getElementById("sortCalendars")
    ?.addEventListener("change", renderCalendarList);
}
