...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadCalendar() {
    try {
      ...
      if (!calRes.ok)
        throw new Error(calData.message || "Failed to fetch calendar");

      calendarColor = calData?.color || "#3a7afe";
      document.documentElement.style.setProperty("--main", calendarColor);
      document.documentElement.style.setProperty(
        "--event-bg",
        lightenColor(calendarColor, 70)
      );
      document.documentElement.style.setProperty(
        "--hover-bg",
        lightenColor(calendarColor, 85)
      );
  ...
  function renderCalendar() {
    ...
    monthYearEl.textContent = currentDate
      .toLocaleString("lv", { month: "long", year: "numeric" })
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (
        events.some(
          (ev) => ev.day === d && ev.month === month + 1 && ev.year === year
        )
      )
  ...
  eventForm.onsubmit = async (e) => {
    ...
    if (!text || !sel) return;
    const day = Number(sel.textContent),
